...
Elm.Application.make = function (_elm) {
   ...
   var repoPanel = function (repo) {
      return function () {
         var rgb = repo.statusChanged ? "red" : "green";
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("col-md-4 col-xs-4")]),
         ...
         _L.fromArray([A2($Html.div,
                      _L.fromArray([$Html$Attributes.$class("panel-heading")
                                   ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                         ,_0: "background"
                                                                         ,_1: rgb}]))]),
                      _L.fromArray([$Html.text(repo.name)]))
                      ,A2($Html.div,
   ...
   var partition = F2(function (n,
   list) {
      return function () {
         var $catch = A2($List.take,
         n,
         list);
         return _U.eq(n,
         $List.length($catch)) ? A2($Basics._op["++"],
         _L.fromArray([$catch]),
         A2(partition,
         n,
         A2($List.drop,
         n,
         list))) : _L.fromArray([$catch]);
      }();
   });
   ...
   var repo = A4($Json$Decode.object3,
   Repository,
   A2($Json$Decode._op[":="],
   "url",
   $Json$Decode.string),
   ...
   "status-changed",
   $Json$Decode.bool));
   var repos = A2($Json$Decode.at,
   _L.fromArray(["repos"]),
   $Json$Decode.list(repo));
   var getRepos = $Effects.task($Task.map(Load)($Task.toResult(A2($Http.get,
   repos,
   "/repos/list"))));
